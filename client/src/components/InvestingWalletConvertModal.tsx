import { useEffect, useMemo, useState } from 'react';

import { formatMoney } from '../lib/formatters';

type ConvertQuote = {
  amountRON: number;
  eurAmount: number;
  rate: {
    eurPerRon: number;
    ronPerEur: number | null;
  };
};

type InvestingWalletConvertModalProps = {
  isOpen: boolean;
  isSubmitting: boolean;
  walletBalanceRON: number;
  quote: ConvertQuote | null;
  isLoadingQuote: boolean;
  quoteError: string | null;
  onAmountChange: (amountRON: number) => void;
  onClose: () => void;
  onSubmit: (amountRON: number) => Promise<void>;
};

const inputClassName =
  'mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-[#2563eb] focus:ring-4 focus:ring-blue-100';

export function InvestingWalletConvertModal({
  isOpen,
  isSubmitting,
  walletBalanceRON,
  quote,
  isLoadingQuote,
  quoteError,
  onAmountChange,
  onClose,
  onSubmit,
}: InvestingWalletConvertModalProps) {
  const [amountRONInput, setAmountRONInput] = useState('0');
  const [formError, setFormError] = useState<string | null>(null);

  useEffect(() => {
    if (!isOpen) {
      return;
    }

    const suggestedAmount = walletBalanceRON > 0 ? String(walletBalanceRON) : '0';
    setAmountRONInput(suggestedAmount);
    setFormError(null);
    onAmountChange(Number(suggestedAmount));
  }, [isOpen, onAmountChange, walletBalanceRON]);

  const parsedAmount = useMemo(() => {
    const numericValue = Number(amountRONInput);
    return Number.isFinite(numericValue) ? numericValue : 0;
  }, [amountRONInput]);

  if (!isOpen) {
    return null;
  }

  const handleSubmit = async () => {
    if (!Number.isFinite(parsedAmount) || parsedAmount <= 0) {
      setFormError('Enter a valid amount greater than zero.');
      return;
    }

    if (parsedAmount > walletBalanceRON) {
      setFormError('Amount exceeds your wallet balance.');
      return;
    }

    setFormError(null);

    try {
      await onSubmit(parsedAmount);
    } catch (error) {
      setFormError(error instanceof Error ? error.message : 'Unable to convert right now.');
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4 py-6 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[1.75rem] bg-white p-6 shadow-[0_24px_60px_rgba(15,23,42,0.18)] sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-[-0.03em] text-slate-900">Convert to EUR</h2>
            <p className="mt-1 text-sm text-slate-500">Move funds from Budget investing wallet (RON) to Invest cash (EUR).</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"
            aria-label="Close"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-5 w-5">
              <path d="m6 6 12 12" />
              <path d="M18 6 6 18" />
            </svg>
          </button>
        </div>

        <label className="mt-6 block text-sm font-medium text-slate-700">
          You send (RON)
          <input
            type="number"
            min="0"
            step="0.01"
            max={walletBalanceRON}
            value={amountRONInput}
            onChange={(event) => {
              setAmountRONInput(event.target.value);
              onAmountChange(Number(event.target.value));
            }}
            className={inputClassName}
          />
        </label>

        <p className="mt-2 text-xs text-slate-500">Available: {formatMoney(walletBalanceRON, 'RON')}</p>

        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
          <p className="text-slate-600">
            Rate:{' '}
            <span className="font-semibold text-slate-900">
              {isLoadingQuote
                ? 'Loading...'
                : quote
                  ? `1 EUR = ${quote.rate.ronPerEur ?? '--'} RON`
                  : '--'}
            </span>
          </p>
          <p className="mt-1 text-slate-600">
            Also:{' '}
            <span className="font-semibold text-slate-900">
              {isLoadingQuote
                ? 'Loading...'
                : quote
                  ? `1 RON = ${quote.rate.eurPerRon} EUR`
                  : '--'}
            </span>
          </p>
          <p className="mt-3 text-slate-600">
            You receive:{' '}
            <span className="font-semibold text-slate-900">
              {isLoadingQuote || !quote ? '--' : formatMoney(quote.eurAmount, 'EUR')}
            </span>
          </p>
        </div>

        {quoteError ? <p className="mt-4 text-sm font-medium text-rose-500">{quoteError}</p> : null}
        {formError ? <p className="mt-2 text-sm font-medium text-rose-500">{formError}</p> : null}

        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button
            type="button"
            onClick={onClose}
            className="inline-flex justify-center rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={() => void handleSubmit()}
            disabled={isSubmitting || isLoadingQuote || parsedAmount <= 0}
            className="inline-flex justify-center rounded-xl bg-[#2563eb] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#1d4ed8] disabled:cursor-not-allowed disabled:bg-blue-300"
          >
            {isSubmitting ? 'Converting...' : 'Convert'}
          </button>
        </div>
      </div>
    </div>
  );
}
