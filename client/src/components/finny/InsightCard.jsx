...
export function InsightCard({ card }) {
  ...
  const blocks = toSafeArray(card.blocks).filter((block) => block && typeof block === 'object');
  const actions = toSafeArray(card.actions).filter((action) => action && typeof action === 'object');
      ...
      {actions.length > 0 ? (
        <div className="mt-5 flex flex-wrap items-center gap-2">
          {actions.map((action, index) => {
            ...
            const variant = action.variant === 'primary' || action.variant === 'secondary' || action.variant === 'link'
              ? action.variant
              : index === 0
                ? 'primary'
                : 'secondary';
