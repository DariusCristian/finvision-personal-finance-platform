  ...
  return (
    <AppShell activeTab="learning">
      <div className="space-y-5">
        ...
        {!payload ? (
          <section className="rounded-[1.5rem] border border-slate-100 bg-white p-8 text-center shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)]">
            <h1 className="text-2xl font-semibold text-slate-900">No quiz result loaded</h1>
            <p className="mt-2 text-slate-500">Start the quiz again to generate fresh results.</p>
            {id ? (
              <Link
                to={`/learn/quizzes/${id}`}
                className="mt-5 inline-flex rounded-xl bg-[#2563eb] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1d4ed8]"
              >
                Retry Quiz
              </Link>
            ) : null}
          ...
          <>
            <section className="rounded-[1.5rem] border border-slate-100 bg-white p-8 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)]">
              <h1 className="text-3xl font-semibold tracking-[-0.04em] text-slate-900">Quiz Results</h1>
              <p className="mt-2 text-slate-500">{payload.quiz.title}</p>

              <div className="mt-6 grid gap-4 md:grid-cols-3">
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-xs uppercase tracking-[0.14em] text-slate-500">Score</p>
                  <p className="mt-2 text-3xl font-bold text-slate-900">{payload.result.score}%</p>
                </div>
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                ...
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                  <p className="text-xs uppercase tracking-[0.14em] text-slate-500">Attempt ID</p>
                  <p className="mt-2 truncate text-sm font-medium text-slate-700">{payload.result.attemptId}</p>
                </div>
              </div>
            </section>

            <section className="rounded-[1.5rem] border border-slate-100 bg-white p-8 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)]">
              <h2 className="text-2xl font-semibold tracking-[-0.03em] text-slate-900">Answer Review</h2>
              <div className="mt-5 space-y-4">
                {feedbackWithQuestionText.map((item) => (
                  <div key={item.questionId} className="rounded-xl border border-slate-100 p-4">
            ...
            {payload.result.recommendedNext ? (
              <section className="rounded-[1.5rem] border border-slate-100 bg-white p-6 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)]">
                <h3 className="text-lg font-semibold text-slate-900">Recommended Next</h3>
                <p className="mt-2 text-sm text-slate-500">Continue with another beginner quiz in the same category.</p>
                <Link
                  to={`/learn/quizzes/${payload.result.recommendedNext.id}`}
                  className="mt-4 inline-flex rounded-xl bg-[#2563eb] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#1d4ed8]"
                >
